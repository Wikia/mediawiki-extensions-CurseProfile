...
class CommentApi extends HydraApiBase {
	...
	public function doAddToDefault() {
		...
		$user = $userFactory->newFromId( $this->getMain()->getVal( 'user_id' ) );
		if ( $user->isAnon() ) {
		...
		$userOptionsLookup = $mediaWikiServices->getUserOptionsLookup();
		if ( $userOptionsLookup->getIntOption( $user, 'comment-pref' ) ) {
	...
	public function doAdd() {
		$mediaWikiServices = MediaWikiServices::getInstance();
		$userFactory = $mediaWikiServices->getUserFactory();
		$toUser = $userFactory->newFromId( $this->getInt( 'user_id' ) );
		if ( !$toUser->isAnon() ) {
